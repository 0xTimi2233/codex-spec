...
每个节点结束后读取 `.agentflow/state.json`、当前 run summary、最新 report 和 review ledger。

## 打回与停止

出现打回或非通过结论时，主线程先读取 report、review ledger 和证据路径，写或更新 `.agentflow/runs/<run-id>/fix-requests/*.md`。若责任角色、允许输入路径和允许输出路径明确，调度对应子代理处理，并在修复后回到对应工作流节点或 review gate。
