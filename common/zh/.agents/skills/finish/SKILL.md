...
4. 将当前 run 产物路径作为 Auditor allowed inputs。追加 Auditor 调度行，调度 Auditor，写入 runtime agent id，并在收到 Auditor 回复后更新该行。
...
8. 执行 `codex-spec archive --run <run-id>`，将完成的 run 移动到不可变归档。
