...
Long-lived project knowledge lives in `agentflow/`: vision, roadmap, ADRs, specs, and test plans. Current work lives in `.agentflow/runs/<run-id>/`: task files, subagent registry, dispatch packets, role reports, review ledgers, fix requests, and summaries. Completed runs are copied to `.agentflow/archives/`.
