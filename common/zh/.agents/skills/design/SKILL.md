...
3. 在 `.agentflow/runs/<run-id>/dispatch-ledger.md` 追加 Architect 调度行，调度 Architect，写入 runtime agent id，并在收到 Architect 回复后更新该行。
4. Architect 写设计/spec/ADR 草案。
5. 写 `.agentflow/runs/<run-id>/dispatch/tester-001.md`。
6. 在 `.agentflow/runs/<run-id>/dispatch-ledger.md` 追加 Tester 调度行，调度 Tester，写入 runtime agent id，并在收到 Tester 回复后更新该行。
