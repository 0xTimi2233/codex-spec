...
17. 确认 `dispatch-ledger.md` 没有 `queued`、`running` 或 `blocked` 行。
18. 执行 `codex-spec-internal archive --run <run-id>`。
19. 用简洁的用户可见 commit message 提交完成的 milestone 变更；若没有文件变化，在 archive 前已写入 summary 的 no-op 记录，不创建空提交。
20. commit 或 no-op 成功后，执行 `codex-spec-internal state set --phase idle --run null --milestone null --blocked false`。
21. 关闭 `dispatch-ledger.md` 中仍打开的 runtime agent id，在 ledger 记录最终状态；后续 milestone 使用新的 dispatch 行和新的子代理。
