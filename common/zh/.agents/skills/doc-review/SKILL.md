...
3. 调度 Doc Reviewer。
4. 在 `.agentflow/runs/<run-id>/dispatch-ledger.md` 追加 Doc Reviewer 调度行；收到 Doc Reviewer 回复后更新该行。
5. Doc Reviewer 写 review report 和 review ledger。
6. 通过时主线程写 `.agentflow/runs/<run-id>/gate.md`，并执行 `codex-spec state set --phase ready-to-execute --run <run-id> --blocked false`。
7. 失败时主线程写 `.agentflow/runs/<run-id>/fix-requests/doc-fix-001.md`，回到 `$design`。
