...
1. 执行 `codex-spec-internal state set --phase designing --run <run-id>`。
2. 调度 Architect 前，确认 current run 和 planning package 已存在。否则停止并建议 `$spec:plan`。
3. 写 `agentflow/runtime/runs/<run-id>/dispatch/architect-001.md`。
...
6. 写 `agentflow/runtime/runs/<run-id>/dispatch/tester-001.md`。
...
10. 写 `agentflow/runtime/runs/<run-id>/dispatch/doc-reviewer-001.md`。
...
13. 失败时写 `agentflow/runtime/runs/<run-id>/fix-requests/doc-fix-<n>.md`，并路由给 Architect、Tester 或 PM 修复。
