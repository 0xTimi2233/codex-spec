...
4. 在 `.agentflow/runs/<run-id>/dispatch-ledger.md` 追加 Auditor 调度行；收到 Auditor 回复后更新该行。
5. Owner 按 dispatch 同步长期文件：PM 同步 roadmap/vision，Architect 同步 ADR/spec，Tester 同步 test-plan。每次 owner 同步都追加并更新调度行。
